...
window.__WIKI_CONTENT_I18N = {
  zh: {
    ...
    "<strong>Server-paginated</strong> table of every recorded session — each page fetches only its slice so cost computation stays bounded no matter how many sessions exist. Case-insensitive search across <code>id</code>, <code>name</code>, and <code>cwd</code> runs server-side with a 300 ms debounce; the status filter composes with search for precise narrowing. Each row shows status badge, agent count, duration, model, and estimated cost. Click any row to drill into the full session detail view with conversation transcript and agent hierarchy.":
      "<strong>服务端分页</strong>的表格，列出每一个已记录的会话——每一页只获取它对应的那一段数据，因此无论存在多少会话，成本计算都保持有界。针对 <code>id</code>、<code>name</code> 和 <code>cwd</code> 的不区分大小写搜索在服务端运行，并带有 300 ms 防抖；状态过滤器可与搜索组合以实现精确缩小范围。每一行显示状态徽章、agent 数量、时长、模型和预估成本。点击任意行即可深入查看完整的会话详情视图，包含对话记录和 agent 层级。",
    "Per-session deep dive with a collapsible agent hierarchy tree and a full chronological event timeline showing every tool call name and summary. An overview panel at the top surfaces tile counters for events, tool calls, subagents, compactions, errors, and duration. Top-tool usage bars and a subagent type breakdown give quick distribution reads. The conversation viewer renders markdown with syntax highlighting and per-tool styled blocks. Export the entire session as JSON or share the permalink for async review.":
      "针对单个会话的深入剖析，配有可折叠的 agent 层级树和一条完整的按时间排序的事件时间线，显示每一次工具调用的名称和摘要。顶部的概览面板展示事件、工具调用、subagent、压缩、错误和时长的磁贴计数器。顶部工具使用条形图和 subagent 类型细分让你快速读取分布情况。对话查看器渲染带语法高亮的 markdown 以及按工具分类的样式化代码块。可将整个会话导出为 JSON，或分享永久链接以供异步审阅。",
    ...
    "Incremental JSONL reader shared across the hook handler, compaction scanner, conversation viewer, and import pipeline. Byte-offset tracking skips already-parsed content; cache hits short-circuit disk I/O so even sessions with tens of thousands of turns stay fast.":
      "增量式 JSONL 读取器，在 hook 处理器、压缩扫描器、对话查看器和导入管道之间共享。字节偏移跟踪会跳过已解析的内容；缓存命中会短路磁盘 I/O，因此即便是有数万个回合的会话也能保持快速。",
    ...
    "Official Claude Code plugin marketplace shipping 5 plugins with 18 skills, 4 agents, and 3 CLI tools. Deep analytics with compaction-aware baselines, productivity automation, developer diagnostics, AI-powered workflow intelligence, and dashboard MCP integration. Install with <code>claude plugin install</code> — no restart needed. Each listing shows author, license, homepage, and per-skill contribution breakdown. The Config Explorer's Plugins tab surfaces installed plugins with live status.":
      "官方 Claude Code 插件市场，发布 5 个插件，包含 18 个技能、4 个智能体和 3 个 CLI 工具。具备压缩感知基线的深度分析、效率自动化、开发者诊断、AI 驱动的工作流智能以及仪表盘 MCP 集成。使用 <code>claude plugin install</code> 安装——无需重启。每个条目都会显示作者、许可证、主页以及按技能的贡献细分。Config Explorer 的 Plugins 标签页会展示已安装的插件及其实时状态。",
    ...
    "A 12-tab inspector at <code>/cc-config</code> for everything Claude Code knows about: skills, subagents, slash commands, output styles, plugins, marketplaces, MCP servers, hooks, settings, memory, keybindings, and statusline scripts. Create, edit, and delete the low-risk text-file surfaces with mandatory timestamped backups before every write. Plugins, MCP, hooks, and live settings stay read-only with explainer banners and copy-able CLI commands. Per-plugin contribution breakdowns show author and license.":
      "位于 <code>/cc-config</code> 的 12 个标签页的检查器，涵盖 Claude Code 所知的一切：技能、子智能体、斜杠命令、输出样式、插件、市场、MCP 服务器、hook、设置、记忆、键位绑定和状态栏脚本。可创建、编辑和删除低风险的文本文件区域，每次写入前都会强制进行带时间戳的备份。插件、MCP、hook 和实时设置保持只读，并配有说明横幅和可复制的 CLI 命令。按插件的贡献细分会显示作者和许可证。",
    ...
    'All endpoints return JSON. Errors follow <code>{ "error": { "code", "message" } }</code>. The full OpenAPI 3.0 spec is served at <code>/api/openapi.json</code> and rendered as interactive Swagger UI at <code>/api/docs</code>.':
      '所有端点都返回 JSON。错误遵循 <code>{ "error": { "code", "message" } }</code> 格式。完整的 OpenAPI 3.0 规范在 <code>/api/openapi.json</code> 提供，并在 <code>/api/docs</code> 渲染为交互式 Swagger UI。',
    ...
    "The Agent Monitor ships with an official Claude Code plugin marketplace containing five production-ready plugins. These extend Claude Code with skills, agents, hooks, CLI tools, and MCP integration — all grounded in the real data model (token tracking with compaction baselines, cost calculation via pattern-matched pricing rules, workflow intelligence with 11 datasets per session, and session metadata including thinking blocks, turn counts, and inference geography).":
      "Agent Monitor 附带一个官方的 Claude Code 插件市场，包含五个生产就绪的插件。它们通过技能、代理、钩子、CLI 工具和 MCP 集成来扩展 Claude Code——全部基于真实的数据模型构建（带压缩基线的令牌跟踪、通过模式匹配定价规则进行的成本计算、每个会话包含 11 个数据集的工作流智能，以及包含思考块、轮次计数和推理地理位置的会话元数据）。",
    Plugin: "插件",
    Skills: "技能",
    Agent: "Agent",
    "CLI Tools": "CLI 工具",
    ...
    "Each plugin follows the official Claude Code plugin specification. The marketplace manifest at <code>.claude-plugin/marketplace.json</code> catalogs all five plugins. Each plugin directory contains:":
      "每个插件都遵循官方的 Claude Code 插件规范。位于 <code>.claude-plugin/marketplace.json</code> 的市场清单编录了全部五个插件。每个插件目录包含：",
  ...
  vi: {
    ...
    "Incremental JSONL reader shared across the hook handler, compaction scanner, conversation viewer, and import pipeline. Byte-offset tracking skips already-parsed content; cache hits short-circuit disk I/O so even sessions with tens of thousands of turns stay fast.":
      "Trình đọc JSONL tăng dần được chia sẻ giữa trình xử lý hook, trình quét nén, trình xem hội thoại và đường ống nhập. Việc theo dõi byte-offset bỏ qua nội dung đã phân tích; các lần trúng cache rút ngắn I/O đĩa nên ngay cả những phiên có hàng chục nghìn lượt vẫn chạy nhanh.",
    "LRU eviction of cold session buffers plus a tail-cap on per-entry growable arrays (turn durations, API errors, compaction entries). A session that runs for days cannot grow a single cache entry without bound, and each entry stores its parsed result only once — no shadow copy.":
    ...
    "Official Claude Code plugin marketplace shipping 5 plugins with 18 skills, 4 agents, and 3 CLI tools. Deep analytics with compaction-aware baselines, productivity automation, developer diagnostics, AI-powered workflow intelligence, and dashboard MCP integration. Install with <code>claude plugin install</code> — no restart needed. Each listing shows author, license, homepage, and per-skill contribution breakdown. The Config Explorer's Plugins tab surfaces installed plugins with live status.":
      "Chợ plugin chính thức của Claude Code phát hành 5 plugin với 18 kỹ năng, 4 agent và 3 công cụ CLI. Phân tích chuyên sâu với các mốc cơ sở có nhận biết nén, tự động hóa năng suất, chẩn đoán cho lập trình viên, trí tuệ quy trình được hỗ trợ bởi AI, và tích hợp MCP của dashboard. Cài đặt bằng <code>claude plugin install</code> — không cần khởi động lại. Mỗi mục liệt kê hiển thị tác giả, giấy phép, trang chủ và phần phân tích đóng góp theo từng kỹ năng. Tab Plugins của Config Explorer hiển thị các plugin đã cài kèm trạng thái trực tiếp.",
    "Spawn <code>claude</code> subprocesses straight from the dashboard with a chat-style streaming UI — multi-turn <b>Conversation</b> or single-shot <b>Headless</b> mode. One-click <b>Resume</b> on any past conversation spawns <code>claude --resume</code> seeded with the prior transcript. Re- attach reconciles in-memory logs with the on-disk JSONL so navigating away never loses history. Slash-command autocomplete, file references, live token/context-window meter, and a thinking-effort dial bring TUI parity to the browser. Same-origin guard blocks drive-by spawns.":
    ...
    "A 12-tab inspector at <code>/cc-config</code> for everything Claude Code knows about: skills, subagents, slash commands, output styles, plugins, marketplaces, MCP servers, hooks, settings, memory, keybindings, and statusline scripts. Create, edit, and delete the low-risk text-file surfaces with mandatory timestamped backups before every write. Plugins, MCP, hooks, and live settings stay read-only with explainer banners and copy-able CLI commands. Per-plugin contribution breakdowns show author and license.":
      "Một trình kiểm tra gồm 12 tab tại <code>/cc-config</code> cho mọi thứ mà Claude Code biết: kỹ năng, subagent, lệnh gạch chéo, kiểu đầu ra, plugin, chợ, máy chủ MCP, hook, cài đặt, bộ nhớ, gán phím và các script thanh trạng thái. Tạo, chỉnh sửa và xóa các bề mặt tệp văn bản rủi ro thấp với việc sao lưu bắt buộc có dấu thời gian trước mỗi lần ghi. Plugin, MCP, hook và cài đặt trực tiếp vẫn ở chế độ chỉ đọc kèm các biểu ngữ giải thích và lệnh CLI có thể sao chép. Phần phân tích đóng góp theo từng plugin hiển thị tác giả và giấy phép.",
    "Mobile-first layouts with stacking grids, horizontally scrollable tables, and a collapsible sidebar that auto-hides below 1400 px. All pages adapt from phone to ultrawide with consistent navigation and readable typography. Kanban columns stack vertically on narrow screens, analytics charts reflow to single-column, and the activity feed stays fully swipeable. Touch targets meet 44 px minimum. Dark theme renders consistently across iOS Safari, Chrome, and Firefox with no flash of unstyled content.":
    ...
    'All endpoints return JSON. Errors follow <code>{ "error": { "code", "message" } }</code>. The full OpenAPI 3.0 spec is served at <code>/api/openapi.json</code> and rendered as interactive Swagger UI at <code>/api/docs</code>.':
      'Tất cả các điểm cuối đều trả về JSON. Lỗi tuân theo định dạng <code>{ "error": { "code", "message" } }</code>. Đặc tả OpenAPI 3.0 đầy đủ được phục vụ tại <code>/api/openapi.json</code> và được hiển thị dưới dạng Swagger UI tương tác tại <code>/api/docs</code>.',
    '<span class="caption-icon">📘</span> <span>Swagger UI at <code>/api/docs</code> — auto-generated interactive playground for every REST endpoint. Try-it-out forms, request/response schema, auth headers, and curl snippets</span>':
    ...
    "The Agent Monitor ships with an official Claude Code plugin marketplace containing five production-ready plugins. These extend Claude Code with skills, agents, hooks, CLI tools, and MCP integration — all grounded in the real data model (token tracking with compaction baselines, cost calculation via pattern-matched pricing rules, workflow intelligence with 11 datasets per session, and session metadata including thinking blocks, turn counts, and inference geography).":
      "Agent Monitor đi kèm một chợ plugin Claude Code chính thức chứa năm plugin sẵn sàng cho sản xuất. Chúng mở rộng Claude Code bằng các kỹ năng, tác tử, hook, công cụ CLI và tích hợp MCP — tất cả đều dựa trên mô hình dữ liệu thực tế (theo dõi token với các đường cơ sở nén, tính toán chi phí qua các quy tắc định giá khớp mẫu, trí tuệ quy trình với 11 tập dữ liệu mỗi phiên, và siêu dữ liệu phiên bao gồm khối suy nghĩ, số lượt và vị trí địa lý suy luận).",
    Plugin: "Plugin",
    Skills: "Kỹ năng",
    Agent: "Agent",
    ...
    "Each plugin follows the official Claude Code plugin specification. The marketplace manifest at <code>.claude-plugin/marketplace.json</code> catalogs all five plugins. Each plugin directory contains:":
      "Mỗi plugin tuân theo đặc tả plugin chính thức của Claude Code. Bản kê chợ tại <code>.claude-plugin/marketplace.json</code> liệt kê tất cả năm plugin. Mỗi thư mục plugin chứa:",
    "All plugins query the Agent Monitor API at <code>http://localhost:4820</code>. Key capabilities they leverage:":
