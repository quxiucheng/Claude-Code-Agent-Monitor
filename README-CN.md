...
3. **服务器** 在 SQLite 事务内处理事件：
   ...
   - **死亡会话存活性回收（liveness reap）** — 退出 Claude Code（Ctrl+C、关闭终端）会触发 `SessionEnd` Hook，但如果此刻仪表盘没有运行，该事件将永远丢失，会话会一直停留在**等待中**，直到废弃清理（默认 3 小时）。同一个 15 秒看门狗通过**进程存活性探测**弥补这一缺口：列出正在运行的 `claude` CLI 进程（macOS 上 `ps` + `lsof`，Linux 上 `/proc`），并将 `cwd` 中不存在任何存活 claude 进程的 `active` 会话标记为完成——落入与真实 `SessionEnd` 相同的 `completed` 状态，并在时间线上留下一条合成的 `SessionEnd` 事件。保护条件：会话的 Transcript 必须至少有 `DASHBOARD_LIVENESS_IDLE_SECONDS`（默认 `60`）未被写入（磁盘上没有 Transcript 时以最后一次 Hook 写入为后备时钟）；探测在 Windows 上、容器内（看不到宿主机进程）、`ps`/`lsof` 失败时，或通过 `DASHBOARD_LIVENESS_PROBE=0` 显式禁用时（Hook 来自另一台机器的场景的逃生阀）会报告"无法回答"（不做任何更改）。误判的完成会自愈：下一个 Hook 事件会重新激活会话。除 15 秒看门狗节奏外，**启动约 5 秒后还会运行一次性回收**，因此仪表盘停机期间死亡的会话会在你第一眼看到 UI 之前被清除
...
| `DASHBOARD_LIVENESS_IDLE_SECONDS` | `60` | 存活性回收的空闲门槛：只有当会话的 Transcript 至少有这么长时间未被写入时（磁盘上没有 Transcript 时以最后一次 Hook 写入为后备时钟），探测才会将其标记为完成，因此回合中或刚 resume 的会话绝不会因一次瞬时的探测偏差而消失——而刚导入的死亡会话（Transcript 旧、DB 行是启动时新写的）仍会在第一轮就被清除 |
