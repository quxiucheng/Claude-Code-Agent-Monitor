...
3. **Máy chủ** xử lý sự kiện bên trong giao dịch SQLite:
   ...
   - **Thu dọn phiên đã chết (liveness reap)** — thoát Claude Code (Ctrl+C, đóng terminal) sẽ kích hoạt hook `SessionEnd`, nhưng nếu dashboard không chạy tại thời điểm đó thì sự kiện bị mất vĩnh viễn và phiên sẽ nằm ở **Đang chờ** cho đến lần quét phiên bỏ rơi (mặc định 3 giờ). Cùng watchdog 15 giây đó lấp khoảng trống bằng một **probe kiểm tra tiến trình**: liệt kê các tiến trình CLI `claude` đang chạy (`ps` + `lsof` trên macOS, `/proc` trên Linux) và hoàn tất mọi phiên `active` mà `cwd` của nó không có tiến trình claude nào còn sống — đưa phiên về đúng trạng thái `completed` như một `SessionEnd` thật, kèm một sự kiện `SessionEnd` tổng hợp trên dòng thời gian. Điều kiện bảo vệ: cả lần ghi hook cuối lẫn mtime của transcript đều phải cũ hơn `DASHBOARD_LIVENESS_IDLE_SECONDS` (mặc định `60`), và probe báo "không có câu trả lời" (không thay đổi gì) trên Windows, trong container (không nhìn thấy tiến trình của host), khi `ps`/`lsof` lỗi, hoặc khi bị tắt qua `DASHBOARD_LIVENESS_PROBE=0` — lối thoát cho các cấu hình mà hook đến từ máy khác. Một lần hoàn tất sai sẽ tự phục hồi: sự kiện hook tiếp theo kích hoạt lại phiên
...
| `DASHBOARD_LIVENESS_IDLE_SECONDS` | `60` | Ngưỡng nhàn rỗi cho cơ chế thu dọn: phiên chỉ bị probe hoàn tất khi **cả** lần ghi hook cuối lẫn mtime của transcript đều cũ ít nhất bằng khoảng này, nên các phiên đang giữa lượt, vừa được nhập hoặc vừa resume không bao giờ biến mất do một lần probe trượt thoáng qua |
