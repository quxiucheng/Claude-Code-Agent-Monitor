...
3. **Server** processes the event inside a SQLite transaction:
   ...
   - **Dead-session liveness reap** — quitting Claude Code (Ctrl+C, closing the terminal) fires a `SessionEnd` hook, but if the dashboard isn't running at that moment the event is lost forever and the session would sit in **Waiting** until the stale sweep (3 h by default). The same 15 s watchdog closes the gap with a **process-liveness probe**: it lists running `claude` CLI processes (`ps` + `lsof` on macOS, `/proc` on Linux) and completes any `active` session whose `cwd` has no live claude process — landing it in the same `completed` state a real `SessionEnd` produces, with a synthetic `SessionEnd` event on the timeline. Guards: both the session's last hook write and its transcript mtime must be older than `DASHBOARD_LIVENESS_IDLE_SECONDS` (default `60`), and the probe reports "no answer" (changing nothing) on Windows, inside containers (host processes are invisible there), when `ps`/`lsof` fail, or when explicitly disabled via `DASHBOARD_LIVENESS_PROBE=0` — the escape hatch for setups where hooks arrive from another machine. A false completion self-heals: the next hook event reactivates the session
...
| `DASHBOARD_LIVENESS_IDLE_SECONDS` | `60` | Idle gate for the liveness reap: a session is only completed by the probe when **both** its last hook write and its transcript mtime are at least this old, so mid-turn, just-imported, or just-resumed sessions never flicker out on a transient probe miss |
