...
3. **Server** processes the event inside a SQLite transaction:
   ...
   - **Dead-session liveness reap** — quitting Claude Code (Ctrl+C, closing the terminal) fires a `SessionEnd` hook, but if the dashboard isn't running at that moment the event is lost forever and the session would sit in **Waiting** until the stale sweep (3 h by default). The same 15 s watchdog closes the gap with a **process-liveness probe**: it lists running `claude` CLI processes (`ps` + `lsof` on macOS, `/proc` on Linux) and completes any `active` session whose `cwd` has no live claude process — landing it in the same `completed` state a real `SessionEnd` produces, with a synthetic `SessionEnd` event on the timeline. Guards: the session's transcript must not have been written for at least `DASHBOARD_LIVENESS_IDLE_SECONDS` (default `60`; the last hook write is the fallback clock when no transcript exists on disk), and the probe reports "no answer" (changing nothing) on Windows, inside containers (host processes are invisible there), when `ps`/`lsof` fail, or when explicitly disabled via `DASHBOARD_LIVENESS_PROBE=0` — the escape hatch for setups where hooks arrive from another machine. A false completion self-heals: the next hook event reactivates the session. Besides the 15 s watchdog cadence, a **one-shot reap runs ~5 s after startup**, so a session that died while the dashboard was down is cleared before your first look at the UI
...
| `DASHBOARD_LIVENESS_IDLE_SECONDS` | `60` | Idle gate for the liveness reap: a session is only completed by the probe when its transcript hasn't been written for at least this long (the last hook write is the fallback clock when no transcript exists on disk), so a mid-turn or just-resumed session never flickers out on a transient probe miss — while a freshly *imported* dead session (old transcript, boot-fresh DB row) clears on the first pass |
